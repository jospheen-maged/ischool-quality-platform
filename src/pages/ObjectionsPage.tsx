...
const reasonOptions = [
  ...
  ['technical_issue', 'A technical issue affected the session'],
  ['history_incorrect', 'Previous flag history was applied incorrectly'],
...
export function ObjectionsPage() {
  ...
  return (
    <div className="page-stack">
      <header className="page-header"><div><p className="eyebrow">Review feedback</p><h1>Objections</h1><p>Evidence-based requests with a traceable decision history.</p></div></header>
      ...
      {profile?.role === 'tutor' && reviewId && (
        <section className="panel form-section">
          ...
          <form className="form-grid" onSubmit={submitObjection}>
            <label>Object to<select value={objectType} onChange={(event) => setObjectType(event.target.value)}><option value="flag">A flag</option><option value="criterion_score">A criterion score</option><option value="comment">A written comment</option><option value="calculation">A calculation</option><option value="complete_review">The complete review</option></select></label>
            ...
            <label className="full-width">Explanation<textarea rows={5} required value={explanation} onChange={(event) => setExplanation(event.target.value)} placeholder="Explain the exact point and include the relevant timestamp or evidence…" /></label>
            <label className="full-width">Requested outcome<textarea rows={2} value={requestedOutcome} onChange={(event) => setRequestedOutcome(event.target.value)} placeholder="For example: reconsider the score or remove the flag." /></label>
