...
export default async function handler(request, response) {
  ...
  let reviewId = null;
  try {
    const { data: evaluator, error: evaluatorError } = await admin
      .from('profiles')
      .select('id, role, is_active')
      .eq('email', 'josphen.maged@ischooltech.com')
      .maybeSingle();
    if (evaluatorError || !evaluator) return response.status(500).json({ stage: 'evaluator_lookup', error: cleanError(evaluatorError) || 'Evaluator not found.' });
