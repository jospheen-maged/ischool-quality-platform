...
function parseBody(body: ApiRequest['body']): InviteBody {
  if (!body) return {};
  if (typeof body === 'string') return JSON.parse(body) as InviteBody;
  return body;
}
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  let tutor: { id: string; full_name: string; email: string | null; user_id: string | null; is_active: boolean } | null = null;
  if (role === 'tutor') {
    if (!tutorId) {
